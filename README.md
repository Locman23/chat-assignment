...
Top-level folders:
- `/server` — Node/Express API, persistence to `server/data.json`.
...
- Use feature branches for each task (e.g., `feature/join-requests`, `fix/persist-data`).
- Make small, focused commits with descriptive messages (what + why).
- Merge to `main` only after verification / smoke tests.
- Tag releases for milestones (e.g., `v0.1-phase1`).
