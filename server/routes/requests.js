...
router.post('/groups/:gid/requests', asyncHandler(async (req, res) => {
...
router.put('/requests/:rid/approve', asyncHandler(async (req, res) => {
...
router.put('/requests/:rid/deny', asyncHandler(async (req, res) => {
  const { rid } = req.params;
  const { requester } = req.body || {};
  const reqUser = await getUserByUsername(requester);
  const isSuper = reqUser && reqUser.roles.includes('Super Admin');
